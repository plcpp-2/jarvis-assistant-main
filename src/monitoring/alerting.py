...
class JarvisMonitoringSystem:
    ...
    def configure_alerting(self):
        ...
        self.alert_rules = {
            'high_cpu_usage': {
                'threshold': 90,
                'severity': 'critical',
                'action': self.send_critical_alert
            },
            'high_memory_usage': {
                'threshold': 85,
                'severity': 'warning',
                'action': self.send_warning_alert
            }
        }
    ...
    def send_critical_alert(self, rule_name: str, current_value: float, threshold: float):
        ...
        logging.critical(
            f"CRITICAL ALERT: {rule_name} exceeded threshold. "
    ...
    def send_warning_alert(self, rule_name: str, current_value: float, threshold: float):
        ...
        logging.warning(
            f"WARNING ALERT: {rule_name} approaching threshold. "
            f"Current: {current_value}%, Threshold: {threshold}%"
        )
