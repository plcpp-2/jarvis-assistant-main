...
async def test_run_inference(ml_executor):
    ...
    result = await ml_executor.run_inference(model_info["model_key"], text)
...
async def test_save_sklearn_model(ml_executor, temp_model_dir):
    ...
    # Save model
    save_info = await ml_executor.save_sklearn_model(model, "test_rf_model", format="joblib")
...
async def test_load_sklearn_model(ml_executor, temp_model_dir):
    ...
    await ml_executor.save_sklearn_model(model, "test_rf_model", format="joblib")

    # Load model
    load_info = await ml_executor.load_sklearn_model("test_rf_model", format="joblib")
...
async def test_batch_inference(ml_executor):
    ...
    result = await ml_executor.run_batch_inference(model_info["model_key"], texts, batch_size=2)
...
async def test_model_optimization(ml_executor):
    ...
    # Optimize model
    opt_info = await ml_executor.optimize_model(model_key, optimization="quantization")
