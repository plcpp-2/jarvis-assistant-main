...
async def language_executor(temp_code_dir):
    ...
    config = {
        "formatters": {"python": "black", "javascript": "prettier", "typescript": "prettier"},
        "linters": {"python": ["pylint", "mypy"], "javascript": ["eslint"], "typescript": ["tslint"]},
    }
...
async def test_code_formatting(language_executor, temp_code_dir):
    ...
    # Python code
    py_code = "def hello(  ):\n  print(   'Hello'   )"
    formatted_py = await language_executor.format_code(py_code, "python")
    assert "def hello():" in formatted_py

    # JavaScript code
    js_code = "function hello(   ){console.log(  'Hello'   )}"
    formatted_js = await language_executor.format_code(js_code, "javascript")
...
async def test_static_analysis(language_executor, temp_code_dir):
    ...
    analysis = await language_executor.analyze_code(py_code, "python")
...
async def test_code_completion(language_executor):
    ...
    completions = await language_executor.get_completions(py_code, "python", position=(7, 11))  # After person.
...
async def test_code_conversion(language_executor):
    ...
    js_code = await language_executor.convert_code(py_code, source_lang="python", target_lang="javascript")
...
async def test_syntax_validation(language_executor):
    ...
    result = await language_executor.validate_syntax(valid_py, "python")
    assert result["valid"] is True

    # Invalid Python code
    invalid_py = "def hello() return 'Hello'"
    result = await language_executor.validate_syntax(invalid_py, "python")
...
async def test_code_metrics(language_executor):
    ...
    metrics = await language_executor.calculate_metrics(py_code, "python")
...
async def test_error_handling(language_executor):
    ...
    with pytest.raises(ValueError):
        await language_executor.format_code("code", "invalid_lang")

    # Invalid code
    with pytest.raises(SyntaxError):
        await language_executor.analyze_code("def invalid syntax", "python")
